...
function displayUsers() {
    ...
    users.forEach(user => {
        usersDisplay.innerHTML += `<p>${user.name} - ${user.email} - ${user.status}</p>`;
    });
}
...
document.getElementById('send-email-form').addEventListener('submit', function(e) {
    ...
    alert(`تم إرسال البريد إلى ${recipient} بموضوع "${subject}". (محاكاة)`);
    this.reset();
});

// إخراج مساعد (للمسؤول فقط)
document.getElementById('remove-btn').addEventListener('click', function() {
    ...
    alert(`تم إخراج الإدمن المساعد ${name}. (محاكاة)`);
});